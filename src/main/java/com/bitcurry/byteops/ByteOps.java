 ...
 * the native <code>byte</code> type in Java. Each bitwise operation is provided 
 * in four different flavors - one for each permutation of the <code>byte</code>
 * and <code>int</code> types. This way the programmer can specify 
 * <code>and(myByteValue, 0xF7)</code> without getting errors about missing method
 * signatures. This library also masks any <code>int</code> values before using them,
 * so performing shifts and rotates will only operate on bits 0 through 7.
 * 
 ...
 * 
 * Note that this class does not <i>eliminate</i> the casts that Java makes from
 * <code>byte</code> to <code>int</code> and visa versa when bitwise operations are
 * performed, it simply hides the operations so the programmer doesn't have to clutter
 * code with them.
 * 
